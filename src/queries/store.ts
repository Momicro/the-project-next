...
export const upsertStore = async (store: Partial<Store>) => {
  try {
    ...
    const { ...storeWithoutUserId } = store;
    console.log("storeWithoutUserId......... ", storeWithoutUserId);

    // Ensure required fields are present for creation
    const requiredFields = ["name", "email", "phone", "url"];
    for (const field of requiredFields) {
      if (!storeWithoutUserId[field as keyof typeof storeWithoutUserId]) {
    ...
    const storeDetails = await db.store.upsert({
      ...
      update: store,
      create: {
        name: storeWithoutUserId.name as string,
        email: storeWithoutUserId.email as string,
        phone: storeWithoutUserId.phone as string,
        url: storeWithoutUserId.url as string,
        description: storeWithoutUserId.description ?? "",
        logo: storeWithoutUserId.logo ?? "",
        cover: storeWithoutUserId.cover ?? "",
        defaultShippingService: storeWithoutUserId.defaultShippingService ?? "",
        defaultShippingFeePerItem:
          storeWithoutUserId.defaultShippingFeePerItem ?? 0,
        defaultShippingFeeForAdditionalItem:
          storeWithoutUserId.defaultShippingFeeForAdditionalItem ?? 0,
        defaultShippingFeePerKg:
          storeWithoutUserId.defaultShippingFeePerKg ?? 0,
        defaultShippingFeeFixed:
          storeWithoutUserId.defaultShippingFeeFixed ?? 0,
        defaultDeliveryTimeMin: storeWithoutUserId.defaultDeliveryTimeMin ?? 0,
        defaultDeliveryTimeMax: storeWithoutUserId.defaultDeliveryTimeMax ?? 0,
        returnPolicy: storeWithoutUserId.returnPolicy ?? "",
        status: storeWithoutUserId.status ?? "PENDING",
        averageRating: storeWithoutUserId.averageRating ?? 0,
        numReviews: storeWithoutUserId.numReviews ?? 0,
        user: {
          connect: { id: user.id },
        },
      },
